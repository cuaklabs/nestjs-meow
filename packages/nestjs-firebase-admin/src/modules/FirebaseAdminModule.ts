...
export class FirebaseAdminModule {
  ...
  public static injectProviders(firebaseTypes: FirebaseType[], appName?: string): DynamicModule {
    ...
    const firebaseTypeFirebaseProviderIdPairs: [FirebaseType, string | FirebaseType][] = firebaseTypes.map(
    ...
    const firebaseAdminModule: DynamicModule = firebaseTypeFirebaseProviderIdPairs.reduce<DynamicModule>(
      (
        dynamicModule: DynamicModule,
        [firebaseType, firebaseProviderId]: [FirebaseType, string | FirebaseType],
      ): DynamicModule => {
        ...
        dynamicModule.providers?.push({
          ...
          useFactory: (firebaseAdminCoreModuleProvider: FirebaseAdminCoreModuleProvider) =>
            firebaseAdminCoreModuleProvider.getProvider(firebaseType, appName),
        });
  ...
  public static forRootAsync(nestFirebaseAdminAppAsyncOptions: NestFirebaseAdminAppAsyncOptions): DynamicModule {
    return {
      imports: [FirebaseAdminCoreModule.forRootAsync(nestFirebaseAdminAppAsyncOptions)],
      module: FirebaseAdminModule,
    };
  }
}
